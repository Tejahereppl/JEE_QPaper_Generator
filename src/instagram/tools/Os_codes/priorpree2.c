...
void priorpree(Process process[],int n){
    ...
    while(completed!=n){
        ...
        if(index==-1){
            ...
            printf("P%d (%d-%d)\n",process[index].process_id,current_time,process[index].burst_time+current_time);
            process[index].completion_time=current_time+process[index].burst_time;
            ...
            current_time+=process[index].burst_time;
